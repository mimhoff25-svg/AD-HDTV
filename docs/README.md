...
### Cable-Style Channels
- Show friendly labels under each video: `Ch N: Name` instead of raw URLs.
- Numeric entry already tunes channels (e.g., typing `3` then `Enter`).
- Manage Channels dialog supports renaming; active player label updates live when the tuned channel is renamed.

### Tokenized HLS Auto-Refresh
...
See `ARCHITECTURE.md` for the lifecycle, state flow, and channel layout.
